                    ...
                    'success': function(data) {
                        if(data['rslt']==0){
                            ...
                            for(var i=0;i<data['paymentls'].length;i++){
                                srlnb++;
                                var str="<div class='col-md-12 col-xs-12 nopadding wrp'><div class='col-md-6 col-xs-6'><mingmu class='pull-left mingmu'>"+data['paymentls'][i]['desc']+"</mingmu></div><div class='col-md-6 col-xs-6'><tm class='pull-right tm'>"+data['paymentls'][i]['tm']+"</tm></div><div class='col-md-12 col-xs-12' style='color:"+data['paymentls'][i]['color']+"'><tm class='pull-right'>"+data['paymentls'][i]['mark']+data['paymentls'][i]['mny']+"</tm></div></div>";//alert(str);return;
                                $('#mainarea').append(str);
                            }
                            nwpg=data['nwpg'];
                            if(data['hasnext']==1){
